...
describe('BoardViewModelService', () => {
  ...
  it('maps normalized coordinates into board percentages', () => {
    const viewModel = service.fromHumans([
      { id: 1, name: 'Ada', tribeId: 'tribe-a', x: 0.25, y: 0.75, busy: false } as any,
    ]);
    ...
    expect(viewModel.markers[0].tribeClass).toBe('tribe-a');
  });

  it('clamps out-of-range coordinates and uses deterministic fallback for null values', () => {
    const viewModel = service.fromHumans([
      { id: 1, name: '', tribeId: 'tribe-a', x: -2, y: 3, busy: false } as any,
      { id: 2, name: null, tribeId: 'tribe-b', x: null, y: null, busy: true } as any,
    ]);
    ...
    expect(viewModel.markers[0].name).toBe('Human 1');
    expect(viewModel.markers[1].name).toBe('Human 2');
    expect(viewModel.markers[1].tribeClass).toBe('tribe-b');
  });
});
